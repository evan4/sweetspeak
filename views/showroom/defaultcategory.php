...
<head itemscope itemtype="http://schema.org/WPHeader">
    ...
    $posts = $model->posts;
    $filter_url = $model->category['info']['category_current']['code'];
...
<div class="wrap wrap_main">
  ...
  <?php 
    ...
    alt="<?=$model->category['category_current']['title']?>">
   <?php } ?>
    <div class="top-main">
      <h1 class="caption caption_h1 top-main__caption <?=$model->category['icon']?>"><?=$model->category['category_current']['title']?></h1>
    ...
    <ul class="top-menu">
      <li class="top-menu__item">
        <span class="top-menu__link top-menu__link_active">Все</span>
      </li>
      <?php 
      
      foreach ($model->category['info']['subcategories'] as $key => $value) { ?>
        <li class="top-menu__item">
          <a href="/<?=$model->category['info']['category_current']['code'].'/'.$value?>" 
          id="tab.dating-<?=$i?>" class="top-menu__link"><?=$key?></a>
        </li>
      <?php } ?>
    </ul>
