<template>
  <div>
    ...
    <b-form @submit.prevent="onSubmit" v-if="templateData">
...
export default {
  ...
  computed: {
     templateData: {
        get: function () {
          const data = this.$store.state.mailings.currentTemplate;
  ...
  methods: {
    onSubmit() {
      const { id, title, body } = this.formData;
      ...
      this.$store.dispatch("mailings/updateTemplate", formData);
    }
  },
  destroyed() {
    this.errors = [];
